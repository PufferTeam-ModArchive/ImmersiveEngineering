...
public class NEIArcFurnaceHandler extends TemplateRecipeHandler {
    ...
    public static NEIArcFurnaceHandler createSubHandler(String subtype) {
        try {
            ...
            String entityProxySubClassName = NEIArcFurnaceHandler.class.getSimpleName()
                .concat(subtype);
            ...
            Class<? extends NEIArcFurnaceHandler> c = (Class<? extends NEIArcFurnaceHandler>) new ProxyClassLoader(
                Thread.currentThread()
                    .getContextClassLoader(),
                cw.toByteArray()).loadClass(entityProxySubClassName.replaceAll("/", "."));
    ...
    public class CachedArcFurnaceRecipe extends CachedRecipe {
        ...
        public CachedArcFurnaceRecipe(ArcFurnaceRecipe recipe) {
            ...
            for (int i = 0; i < recipe.additives.length; i++) if (recipe.additives[i] != null) {
    ...
    boolean specialTypeCompare(ArcFurnaceRecipe r) {
        ...
        if (this.specialRecipeType != null && r.specialRecipeType != null
    ...
    public String getRecipeName() {
        return StatCollector.translateToLocal("tile.ImmersiveEngineering.metalMultiblock.arcFurnace.name")
            + (this.specialRecipeType != null && !this.specialRecipeType.isEmpty() ? (": " + this.specialRecipeType)
                : "");
    }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        if (result != null) for (ArcFurnaceRecipe r : ArcFurnaceRecipe.recipeList) if (r != null && r.input != null
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        if (ingredient != null) for (ArcFurnaceRecipe r : ArcFurnaceRecipe.recipeList)
            if (r != null && r.input != null && specialTypeCompare(r)) {
                if (Utils.stackMatchesObject(ingredient, r.input)) this.arecipes.add(new CachedArcFurnaceRecipe(r));
                else for (Object o : r.additives)
                    if (Utils.stackMatchesObject(ingredient, o)) this.arecipes.add(new CachedArcFurnaceRecipe(r));
            }
    }
