...
public class ShaderRegistry {
    ...
    /**
     * A list of shader names that can generate in chests/crates. Names are added multiple times depending on their
     * weight
     */
    ...
    /**
     * A map of player names to received shaders. Saved with worlddata. Designed to prioritize shaders the player has
     * not yet received
     */
    ...
    /**
     * The total weight in relation to the player. This takes into account shaders the player has gotten, which then
     * result in less weight
     */
    ...
    public static ShaderCase getShader(String name, String shaderType) {
        if (shaderRegistry.containsKey(name)) return shaderRegistry.get(name)
            .getCase(shaderType);
        return null;
    }

    public static ShaderRegistryEntry registerShader(String name, String overlayType, EnumRarity rarity,
        int[] colourPrimary, int[] colourSecondary, int[] colourBackground, int[] colourBlade, String additionalTexture,
        boolean loot, boolean bags) {
    ...
    public static ShaderCaseRevolver registerShader_Revolver(String name, String overlayType, EnumRarity rarity,
        int[] colourGrip, int[] colourPrimary, int[] colourSecondary, int[] colourBlade, String additionalTexture) {
        ...
        else shaderRegistry.get(name)
            .addCase(shader);
        return shader;
    }

    public static ShaderCaseChemthrower registerShader_Chemthrower(String name, String overlayType, EnumRarity rarity,
        int[] colourGrip, int[] colourPrimary, int[] colourSecondary, boolean cageOnBase, boolean tanksUncoloured,
    ...
    public static ShaderCaseDrill registerShader_Drill(String name, String overlayType, EnumRarity rarity,
        int[] colourGrip, int[] colourPrimary, int[] colourSecondary, String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseDrill shader = new ShaderCaseDrill(
            overlayType,
            colourGrip,
            colourPrimary,
            colourSecondary,
    ...
    public static ShaderCaseRailgun registerShader_Railgun(String name, String overlayType, EnumRarity rarity,
        int[] colourGrip, int[] colourPrimary, int[] colourSecondary, String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseRailgun shader = new ShaderCaseRailgun(
            overlayType,
            colourGrip,
            colourPrimary,
            colourSecondary,
    ...
    public static ShaderCaseMinecart registerShader_Minecart(String name, String overlayType, EnumRarity rarity,
        int[] colourUnderlying, int[] colourPrimary, int[] colourSecondary, String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
    ...
    public static ShaderCaseBalloon registerShader_Balloon(String name, String overlayType, EnumRarity rarity,
        int[] colourUnderlying, int[] colourPrimary, int[] colourSecondary, String additionalTexture) {
        if (!shaderList.contains(name)) shaderList.add(name);
        ShaderCaseBalloon shader = new ShaderCaseBalloon(
            overlayType,
            colourUnderlying,
            colourPrimary,
            colourSecondary,
    ...
    public static void compileWeight() {
        ...
        Collections.sort(sortedRarityMap, new Comparator<EnumRarity>() {
            ...
            public int compare(EnumRarity enum0, EnumRarity enum1) {
                return Integer
                    .compare(ShaderRegistry.rarityWeightMap.get(enum0), ShaderRegistry.rarityWeightMap.get(enum1));
            }
        });
        ...
        if (manualEntry != null) {
            ...
            for (int i = 0; i < ShaderRegistry.sortedRarityMap.size(); i++) {
                ...
                shaderBags[i].getTagCompound()
                    .setString("rarity", outputRarity.toString());
                ...
                if (!upperRarities.isEmpty()) {
                    ...
                    s0.getTagCompound()
                        .setString("rarity", outputRarity.toString());
                    if (!inputList.isEmpty()) recipes.add(
                        new PositionedItemStack[] { new PositionedItemStack(inputList, 33, 0),
                    ...
                    for (ShaderRegistryEntry entry : ShaderRegistry.shaderRegistry.values())
                        if (upperRarities.contains(entry.getRarity())) {
                            ...
                            shader.getTagCompound()
                                .setString("shader_name", entry.getName());
    ...
    public static void recalculatePlayerTotalWeight(String player) {
        ...
        else playerTotalWeight.get(player)
            .clear();
        List<String> received = receivedShaders.get(player);
        for (ShaderRegistryEntry entry : shaderRegistry.values()) if (entry.getIsBagLoot()) {
            int entryRarityWeight = rarityWeightMap.get(entry.getRarity());
            for (Map.Entry<EnumRarity, Integer> weightedRarity : rarityWeightMap.entrySet())
                if (entry.getIsInLowerBags() ? (weightedRarity.getValue() >= entryRarityWeight)
                    : (weightedRarity.getValue() == entryRarityWeight)) {
                        int weight = playerTotalWeight.get(player)
                            .containsKey(weightedRarity.getKey())
                                ? playerTotalWeight.get(player)
                                    .get(weightedRarity.getKey())
                                : 0;
                        int value = entry.getWeight();
                        if (received.contains(entry.getName())) value = 1;
                        playerTotalWeight.get(player)
                            .put(weightedRarity.getKey(), weight + value);
                    }
        }
    }
    ...
    public static String getRandomShader(String player, Random rand, EnumRarity minRarity, boolean addToReceived) {
        ...
        total = playerTotalWeight.get(player)
            .get(minRarity);
        ...
        for (ShaderRegistryEntry entry : shaderRegistry.values()) if (entry.getIsBagLoot()) {
            ...
            if (entry.getIsInLowerBags() ? (minWeight >= entryRarityWeight) : (minWeight == entryRarityWeight)) {
                int value = entry.getWeight();
                if (receivedShaders.get(player)
                    .contains(entry.getName())) value = 1;
                weight -= value;
                if (weight <= 0) {
        ...
        if (addToReceived && shader != null && !shader.isEmpty() && player != null && !player.isEmpty()) {
            if (!receivedShaders.get(player)
                .contains(shader)) receivedShaders.put(player, shader);
