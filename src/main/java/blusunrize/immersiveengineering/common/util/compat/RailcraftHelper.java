...
public class RailcraftHelper extends IECompatModule {
    ...
    public void init() {
        ...
        RailgunHandler.registerProjectileProperties(new ItemStack(itemRail, 1, 1), 6, 1.375)
            .setColourMap(
                new int[][] { { 0xa4a4a4, 0xa4a4a4, 0x686868, 0xddb82c, 0xc9901f },
    ...
    public void postInit() {
        if (FMLCommonHandler.instance()
            ...
            try {
                ...
                for (Map.Entry<Class, ModelBase> e : modelMap.entrySet()) if (e.getValue()
                    .getClass()
                    .getName()
                    .endsWith("ModelLowSidesMinecart")) e.setValue(new ModelShaderLowSidesMinecart());

                ShaderCaseMinecart.invalidMinecartClasses.add(
                    (Class<? extends EntityMinecart>) Class.forName("mods.railcraft.common.carts.EntityLocomotive"));
                ShaderCaseMinecart.invalidMinecartClasses.add(
                    (Class<? extends EntityMinecart>) Class.forName("mods.railcraft.common.carts.EntityTunnelBore"));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public static boolean isSteelSlab(TileEntity tile) {
        try {
            return (!(boolean) c_isDoubleSlab.invoke(tile) && c_getBottomSlab.invoke(tile)
                .toString() == "STEEL");
        } catch (Exception e) {}
        return false;
    }
