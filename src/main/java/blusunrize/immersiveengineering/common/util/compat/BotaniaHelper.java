...
public class BotaniaHelper extends IECompatModule {
    ...
    @Override
    public void init() {
        if (Config.getBoolean("hardmodeBulletRecipes")) BlueprintCraftingRecipe.addRecipe(
            "specialBullet",
            new ItemStack(IEContent.itemBullet, 1, 7),
            new ItemStack(IEContent.itemBullet, 1, 0),
            Items.gunpowder,
            "ingotTerrasteel");
        else BlueprintCraftingRecipe.addRecipe(
            "specialBullet",
            new ItemStack(IEContent.itemBullet, 1, 7),
            new ItemStack(IEContent.itemBullet, 1, 0),
            Items.gunpowder,
        ...
        if (Utils.getModVersion("Botania")
            .startsWith("r1.8")) {
        ...
        if (FMLCommonHandler.instance()
            .getEffectiveSide() == Side.CLIENT) MinecraftForge.EVENT_BUS.register(this);
    }
    ...
    public static class ThreadContributorToNameFormatter extends Thread {
        ...
        public void run() {
            ...
            for (String uuid : ItemRevolver.specialRevolvers.keySet()) nameToSpecial.putAll(
                ImmersiveEngineering.proxy.getNameFromUUID(uuid)
                    .toLowerCase(),
    ...
    public void onLivingDrops(LivingDropsEvent event) {
        if (!event.isCanceled() && event.entityLiving.getClass()
            .getName()
            .endsWith("EntityDoppleganger")) {
            NBTTagCompound tag = new NBTTagCompound();
            event.entityLiving.writeEntityToNBT(tag);
            if (tag.getBoolean("hardMode"))
                for (EntityItem item : event.drops) if (item != null && item.getEntityItem() != null
    ...
    public void onPotatoRender(TinyPotatoRenderEvent event) {
        ...
        if (revolverEntity == null) {
            revolverEntity = new EntityItem(
                event.tile.getWorldObj(),
                0.0D,
                0.0D,
                0.0D,
                new ItemStack(IEContent.itemRevolver));
            ...
            if (formattedName.equalsIgnoreCase("Mr Damien Hazard") || event.name.equalsIgnoreCase("Mr Hazard"))
                ...
                if (nameToSpecial.containsKey(event.name.toLowerCase())) {
                    ...
                    if (list != null && list.size() > 0) {
                        long ticks = event.tile.getWorldObj() != null ? event.tile.getWorldObj()
                            .getTotalWorldTime() / 100 : 0;
