...
public class ArcRecyclingThreadHandler {
    ...
    public static void doRecipeProfiling() {
        ...
        int r = 0;
        if (hasProfiled) while (prevRecipeIt.hasNext()) if ("Recycling".equals(prevRecipeIt.next().specialRecipeType)) {
            prevRecipeIt.remove();
            r++;
        }
        ...
        for (int i = 0; i < threads.length; i++) {
            ...
            try {
                thread.join();
                for (RecyclingCalculation calc : thread.calculatedOutputs) if (calc.isValid()) validated.add(calc);
                else {
        ...
        while (!nonValidated.isEmpty() && timeout++ < (invalidCount * 10)) {
            ...
            for (RecyclingCalculation valid : validated) {
                Iterator<Map.Entry<ItemStack, RecyclingCalculation>> itNonValid = nonValidated.entries()
                    .iterator();
            ...
            nonValidated.values()
                .removeAll(newlyValid);
            validated.addAll(newlyValid);
        }
        // HashSet to avoid duplicates
        HashSet<String> finishedRecycles = new HashSet<String>();
        for (RecyclingCalculation valid : validated) if (finishedRecycles.add(valid.stack.toString()))
        ...
        for (RecyclingCalculation invalid : Sets.newHashSet(nonValidated.values()))
            if (finishedRecycles.add(invalid.stack.toString())) {
                IELogger.info(
                    "Couldn't fully analyze " + invalid.stack
        ...
        IELogger.info(
            "Finished recipe profiler for Arc Recycling, took " + (System.currentTimeMillis() - timestamp)
                + " milliseconds");
        hasProfiled = true;
    }
    ...
    public static RecyclingCalculation getRecycleCalculation(ItemStack stack, IRecipe recipe) {
        ...
        else if (recipe instanceof ShapelessOreRecipe) inputs = ((ShapelessOreRecipe) recipe).getInput()
            .toArray();
        ...
        if (inputs != null) {
            ...
            for (Object in : inputs) if (in != null) {
                ...
                if (brokenDown[0] != null && brokenDown[0] instanceof ItemStack
                    && brokenDown[1] != null
            ...
            if (!outputs.isEmpty() || !missingSub.isEmpty()) {
                RecyclingCalculation calc = new RecyclingCalculation(
                    recipe,
                    Utils.copyStackWithAmount(stack, 1),
                    outputs);
        ...
        public boolean validateSubcomponent(RecyclingCalculation calc) {
            ...
            while (it.hasNext()) {
                ...
                if (OreDictionary.itemMatches(next, calc.stack, false)) {
                    for (Map.Entry<ItemStack, Double> e : calc.outputs.entrySet()) if (outputs.containsKey(e.getKey()))
