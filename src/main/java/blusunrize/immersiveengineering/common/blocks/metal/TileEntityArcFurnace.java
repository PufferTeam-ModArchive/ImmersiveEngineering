...
public class TileEntityArcFurnace extends TileEntityMultiblockPart implements IEnergyReceiver, ISidedInventory {
    ...
    public void updateEntity() {
        ...
        if (worldObj.isRemote) {
            ...
            if (active) for (int i = 0; i < 4; i++) {
                if (worldObj.rand.nextInt(6) == 0) ImmersiveEngineering.proxy.spawnSparkFX(
                    worldObj,
        ...
        if (!enabled) {
            ...
            if (hasElectrodes) {
                ...
                for (int i = 0; i < 12; i++) {
                    ...
                    if (recipe != null) {
                            ...
                            for (int iOut = 0; iOut < outputs.length; iOut++) {
                                ...
                                for (int j = 16; j < 22; j++) {
                                    ...
                                    if (inventory[j] != null && !OreDictionary.itemMatches(inventory[j], out, false))
                                        ...
                                        && !OreDictionary.itemMatches(outUpToNow[j - 16], out, false)) matches = false;
                        ...
                        if (recipe.slag != null
                            && !(inventory[22] == null || (OreDictionary.itemMatches(inventory[22], recipe.slag, false)
                                && inventory[22].stackSize + recipe.slag.stackSize <= inventory[22].getMaxStackSize())))
                            continue;
                        ...
                        if (processMax[i] != recipe.time) {
                            ...
                            if (process[i] >= processMax[i]) {
                                ...
                                for (Object add : recipe.additives) {
                                    ...
                                    for (int j = 12; j < 16; j++) if (this.getStackInSlot(j) != null
            ...
            if (worldObj.getTotalWorldTime() % 8 == 0) {
                ...
                for (int j = 16; j < 22; j++) if (this.getStackInSlot(j) != null) {
                    ...
                    if ((inventoryFront instanceof ISidedInventory && ((ISidedInventory) inventoryFront)
                        ...
                        stack = Utils.insertStackIntoInventory(
                            (IInventory) inventoryFront,
                            stack,
                            ForgeDirection.OPPOSITES[facing]);
                    if (stack == null) {
                ...
                if (this.getStackInSlot(22) != null) {
                    ...
                    if ((inventoryBack instanceof ISidedInventory && ((ISidedInventory) inventoryBack)
                        .getAccessibleSlotsFromSide(ForgeDirection.OPPOSITES[facing]).length > 0)
    ...
    public boolean receiveClientEvent(int id, int arg) {
        ...
        if (id == 0 && FMLCommonHandler.instance()
            .getEffectiveSide() == Side.CLIENT) if (this.pouringMetal <= 0) this.pouringMetal = arg;
        return true;
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (renderAABB == null) if (pos == 62) renderAABB = AxisAlignedBB
    ...
    public float[] getBlockBounds() {
        ...
        if (pos < 25) {
            if (pos == 1 || pos == 3) return new float[] { fl == 4 ? .4375f : 0, 0, fl == 2 ? .4375f : 0,
                fl == 5 ? .5625f : 1, .5f, fl == 3 ? .5625f : 1 };
            ...
            return new float[] { fl == 3 ? .125f : (fl == 2 ? .625f : 0), .125f,
                fl == 4 ? .125f : (fl == 5 ? .625f : 0), fl == 2 ? .875f : (fl == 3 ? .375f : 1), 0.375F,
                fl == 5 ? .875f : (fl == 4 ? .375f : 1) };
        } else if (pos < 50) {
            if (pos % 5 == 0) return new float[] { fw == 3 ? .125f : 0, 0, fw == 4 ? .125f : 0, fw == 2 ? .875f : 1,
                pos % 25 / 5 == 3 ? .5f : 1, fw == 5 ? .875f : 1 };
            else if (pos % 5 == 4) return new float[] { fw == 2 ? .125f : 0, 0, fw == 5 ? .125f : 0,
                fw == 3 ? .875f : 1, pos % 25 / 5 == 3 ? .5f : 1, fw == 4 ? .875f : 1 };
            else if (pos == 43) return new float[] { fl == 4 || fl == 3 ? 0 : .125f, 0.5f,
                fl == 2 || fl == 4 ? 0 : .125f, fl == 5 || fl == 2 ? 1 : .875f, 1, fl == 3 || fl == 5 ? 1 : .875f };
            else if (pos == 41) return new float[] { fl == 3 || fl == 4 ? 0 : .125f, 0.5f,
                fl == 2 || fl == 5 ? 0 : .125f, fl == 2 || fl == 5 ? 1 : .875f, 1, fl == 4 || fl == 3 ? 1 : .875f };
            ...
            else if (pos == 60) return new float[] { fw == 3 ? .125f : 0, 0, fw == 4 ? .125f : 0, fw == 2 ? .875f : 1,
                1, fw == 5 ? .875f : 1 };
            else if (pos == 64) return new float[] { fw == 2 ? .375f : 0, 0, fw == 5 ? .375f : 0, fw == 3 ? .625f : 1,
                1, fw == 4 ? .625f : 1 };
            else if (pos == 70)
                return new float[] { fw == 3 ? .5f : 0, 0, fw == 4 ? .5f : 0, fw == 2 ? .5f : 1, 1, fw == 5 ? .5f : 1 };
            else if (pos == 74)
        ...
        } else if (pos == 112) return new float[] { fl == 4 ? .0625f : 0, 0, fl == 2 ? .0625f : 0, fl == 5 ? .9375f : 1,
            1, fl == 3 ? .9375f : 1 };
        else if (pos == 117) return new float[] { 0, .6875f, 0, 1, 1, 1 };
        else if (pos % 25 / 5 == 4) {
            if (pos % 5 == 1)
                return new float[] { fw == 3 ? .5f : 0, 0, fw == 4 ? .5f : 0, fw == 2 ? .5f : 1, 1, fw == 5 ? .5f : 1 };
            else if (pos % 5 == 2) return new float[] { fl == 5 ? .25f : 0, 0, fl == 3 ? .25f : 0, fl == 4 ? .75f : 1,
                1, fl == 2 ? .75f : 1 };
            else if (pos % 5 == 3)
                return new float[] { fw == 2 ? .5f : 0, 0, fw == 5 ? .5f : 0, fw == 3 ? .5f : 1, 1, fw == 4 ? .5f : 1 };
        }
    ...
    public void invalidate() {
        ...
        if (formed && !worldObj.isRemote) {
            ...
            for (int h = -2; h <= 2; h++) for (int l = -2; l <= 2; l++) for (int w = -2; w <= 2; w++) {
    ...
    public int[] getAccessibleSlotsFromSide(int side) {
        ...
        if ((pos == 86 || pos == 88) && side == 1) // Input hatches on top
        {
            ...
            for (int i = 0; i <= 11; i++) if (master.getStackInSlot(i) == null) {
                slotsMain.add(i);
                allOccupied = false;
            }
            ...
            Collections.sort(slotsMain, new Comparator<Integer>() {
                ...
                public int compare(Integer arg0, Integer arg1) {
                    ...
                    return Integer
                        .compare((stack0 != null ? stack0.stackSize : 0), (stack1 != null ? stack1.stackSize : 0));
                }
            });

            ArrayList<Integer> slotsAdditives = new ArrayList<>(4);
            allOccupied = true;
            for (int i = 12; i <= 15; i++) if (master.getStackInSlot(i) == null) {
