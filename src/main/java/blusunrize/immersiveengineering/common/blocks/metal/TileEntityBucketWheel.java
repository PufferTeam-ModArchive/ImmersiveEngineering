...
public class TileEntityBucketWheel extends TileEntityMultiblockPart {
    ...
    public void readCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        rotation = (Math.abs(nbtRot - rotation) > 5 * (float) Config.getDouble("excavator_speed")) ? nbtRot : rotation; // avoid
                                                                                                                        // stuttering
                                                                                                                        // due
                                                                                                                        // to
                                                                                                                        // packet
    ...
    public void invalidate() {
        ...
        if (formed && !worldObj.isRemote) {
            ...
            for (int w = -3; w <= 3; w++) for (int h = -3; h <= 3; h++) {
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (renderAABB == null) if (pos == 24) renderAABB = AxisAlignedBB.getBoundingBox(
            xCoord - (facing < 4 ? 3 : 0),
            yCoord - 3,
            zCoord - (facing > 3 ? 3 : 0),
            xCoord + (facing < 4 ? 4 : 1),
    ...
    public float[] getBlockBounds() {
        ...
        else if (pos == 21) return new float[] { facing == 2 ? .25f : 0, 0, facing == 4 ? .25f : 0,
            facing == 3 ? .75f : 1, 1, facing == 5 ? .75f : 1 };
        else if (pos == 27) return new float[] { facing == 3 ? .25f : 0, 0, facing == 5 ? .25f : 0,
            facing == 2 ? .75f : 1, 1, facing == 4 ? .75f : 1 };
        else if (pos == 15 || pos == 29) return new float[] { facing == 2 ? .25f : 0, 0, facing == 4 ? .25f : 0,
            facing == 3 ? .75f : 1, 1, facing == 5 ? .75f : 1 };
        else if (pos == 19 || pos == 33) return new float[] { facing == 3 ? .25f : 0, 0, facing == 5 ? .25f : 0,
            facing == 2 ? .75f : 1, 1, facing == 4 ? .75f : 1 };
