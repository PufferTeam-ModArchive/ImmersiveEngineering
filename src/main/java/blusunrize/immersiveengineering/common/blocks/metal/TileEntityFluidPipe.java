...
public class TileEntityFluidPipe extends TileEntityIEBase implements IFluidHandler, IColouredTile {
    ...
    static {
        TileEntityFluidPipe.validScaffoldCoverings
            .add(new ItemStack(IEContent.blockMetalDecoration, 1, BlockMetalDecoration.META_scaffolding));
        TileEntityFluidPipe.validScaffoldCoverings
            .add(new ItemStack(IEContent.blockMetalDecoration, 1, BlockMetalDecoration.META_scaffolding2));
        TileEntityFluidPipe.validScaffoldCoverings
            .add(new ItemStack(IEContent.blockMetalDecoration, 1, BlockMetalDecoration.META_aluminiumScaffolding));
        TileEntityFluidPipe.validScaffoldCoverings
            .add(new ItemStack(IEContent.blockMetalDecoration, 1, BlockMetalDecoration.META_aluminiumScaffolding2));
    ...
    public static Set<DirectionalFluidOutput> getConnectedFluidHandlers(ChunkCoordinates node, World world) {
        ...
        Set<DirectionalFluidOutput> fluidHandlers = Collections
                ...
                if (!closedList.contains(next)
                    ...
                    for (int i = 0; i < 6; i++) {
                        ...
                        if (b) {
                            ...
                            if (world
                                .blockExists(next.posX + fd.offsetX, next.posY + fd.offsetY, next.posZ + fd.offsetZ)) {
                                TileEntity te2 = world.getTileEntity(
                                    next.posX + fd.offsetX,
                                    next.posY + fd.offsetY,
                                    next.posZ + fd.offsetZ);
                                if (te2 instanceof TileEntityFluidPipe) openList.add(
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        ...
        ArrayList<DirectionalFluidOutput> outputList = new ArrayList(
            getConnectedFluidHandlers(new ChunkCoordinates(xCoord, yCoord, zCoord), worldObj));
        if (outputList.size() < 1) return 0;
        ChunkCoordinates ccFrom = new ChunkCoordinates(
            xCoord + from.offsetX,
            yCoord + from.offsetY,
            zCoord + from.offsetZ);
        int sum = 0;
        ...
        for (DirectionalFluidOutput output : outputList) {
            ...
            if (!cc.equals(ccFrom) && worldObj.blockExists(cc.posX, cc.posY, cc.posZ)
                ...
                int limit = (resource.tag != null && resource.tag.hasKey("pressurized"))
                    || canOutputPressurized(output.output, false) ? 1000 : 50;
        ...
        if (sum > 0) {
            ...
            for (DirectionalFluidOutput output : sorting.keySet()) {
                int amount = Math.min(sorting.get(output), canAccept); // fixes issue #1236 on
                                                                       // BluSunrize/ImmersiveEngineering
                int r = output.output.fill(
                    output.direction.getOpposite(),
                    Utils.copyFluidStackWithAmount(resource, amount, true),
                    doFill);
    ...
    public byte getConnectionByte() {
        ...
        for (int i = 5; i >= 0; i--) {
            ...
            if (sideConfig[i] == 0 && con instanceof IFluidHandler) {
                tankInfo = ((IFluidHandler) con).getTankInfo(
