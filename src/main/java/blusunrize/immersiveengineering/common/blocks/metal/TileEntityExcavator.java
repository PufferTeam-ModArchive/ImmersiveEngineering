...
public class TileEntityExcavator extends TileEntityMultiblockPart implements IEnergyReceiver {
    ...
    public void updateEntity() {
        ...
        int[] wheelAxis = { xCoord + (facing == 5 ? -4 : facing == 4 ? 4 : 0), yCoord,
            zCoord + (facing == 3 ? -4 : facing == 2 ? 4 : 0) };
        ...
        if (!worldObj.isRemote) {
            if (center instanceof TileEntityBucketWheel) {
                if (((TileEntityBucketWheel) center).facing == ff) {
                    ...
                    if (active != wheel.active) worldObj.addBlockEvent(
                        wheel.xCoord,
                        wheel.yCoord,
                        wheel.zCoord,
                        wheel.getBlockType(),
            ...
            ExcavatorHandler.MineralMix mineral = ExcavatorHandler
                .getRandomMineral(worldObj, wheelAxis[0] >> 4, wheelAxis[2] >> 4);
            boolean enabled;
                        ...
                        if (wheel.digStacks[(target + 4) % 8] == null) {
                            ...
                            if (blocking != null) {
                                ...
                                && !worldObj.isAirBlock(wheel.xCoord, wheel.yCoord - 5, wheel.zCoord)) {
                                    ...
                                    if (ore != null && configChance > Config.getDouble("excavator_chance")
        ...
        if (block != null && !worldObj.isAirBlock(x, y, z)
            ...
            if (block.removedByPlayer(worldObj, fakePlayer, x, y, z, true)) {
                ...
                if (block.canSilkHarvest(worldObj, fakePlayer, x, y, z, meta)) {
                    ...
                    ForgeEventFactory
                        .fireBlockHarvesting(items, worldObj, block, x, y, z, meta, 0, 1.0f, true, fakePlayer);
    ...
    public void invalidate() {
        ...
        if (formed && !worldObj.isRemote) {
            ...
            int[] wheelAxis = { startX + (facing == 5 ? -4 : facing == 4 ? 4 : 0), startY,
    ...
    public boolean canConnectEnergy(ForgeDirection from) {
        return formed && (pos == 11 || pos == 14 || pos == 17)
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (renderAABB == null) if (pos == 4) renderAABB = AxisAlignedBB.getBoundingBox(
    ...
    public float[] getBlockBounds() {
        ...
        if (pos == 42) return new float[] { fl == 4 || fl == 5 ? -.5f : 0, 0, fl == 2 || fl == 3 ? -.5f : 0,
            fl == 4 || fl == 5 ? 1.5f : 1, .5f, fl == 2 || fl == 3 ? 1.5f : 1 };
        else if (pos == 29 || pos == 38 || pos == 47)
            return new float[] { fw == 2 ? .25f : 0, 0, fw == 5 ? .25f : 0, fw == 3 ? .75f : 1, 1, fw == 4 ? .75f : 1 };
        else if (pos == 44) return new float[] { fw == 3 ? .875f : 0, 0, fw == 4 ? .875f : 0, fw == 2 ? .125f : 1, 1,
            fw == 5 ? .125f : 1 };
        else if (pos == 35)
            return new float[] { fl == 5 ? .375f : fl == 4 ? .5f : 0, 0, fl == 3 ? .375f : fl == 2 ? .5f : 0,
                fl == 5 ? .5f : fl == 4 ? .625f : 1, 1, fl == 3 ? .5f : fl == 2 ? .625f : 1 };
        else if (pos == 53)
            return new float[] { fl == 4 ? .375f : fl == 5 ? .5f : 0, 0, fl == 2 ? .375f : fl == 3 ? .5f : 0,
                fl == 4 ? .5f : fl == 5 ? .625f : 1, 1, fl == 2 ? .5f : fl == 3 ? .625f : 1 };
