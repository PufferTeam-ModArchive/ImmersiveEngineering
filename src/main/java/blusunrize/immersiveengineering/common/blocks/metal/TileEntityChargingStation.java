...
public class TileEntityChargingStation extends TileEntityIEBase implements IEnergyReceiver, IInventory {
    ...
    public void updateEntity() {
            ...
            if (worldObj.isRemote && charging) {
                ...
                for (int i = 0; i < max; i++) {
                    ...
                    if (charge >= 1 || (time % 12 >= i * 4 && time % 12 <= i * 4 + 2)) {
                        ...
                        ImmersiveEngineering.proxy
                            .spawnRedstoneFX(worldObj, x, y, z, .25, .25, .25, .5f, 1 - charge, charge, 0);
                    }
                }
            } else if (charging) {
                ...
                if (inventory.getItem() instanceof IEnergyContainerItem) {
                    ...
                    if (max > 0 && space > 0) {
                        ...
                        double insert = Math
                            .min(space, ModCompatability.convertRFtoEU(Config.getInt("charger_consumption"), 5));
                        double accepted = IC2Helper.chargeItem(inventory, insert, true);
                        int rfAccepted = this.energyStorage
                            .extractEnergy(ModCompatability.convertEUtoRF(accepted), false);
                        if (rfAccepted > 0)
        ...
        if (!worldObj.isRemote && worldObj.getTotalWorldTime() % 32 == ((xCoord ^ zCoord) & 31)) {
            ...
            if (inventory != null && (inventory.getItem() instanceof IEnergyContainerItem
                ...
                } else charge = (float) (IC2Helper.getCurrentItemCharge(inventory)
                    / IC2Helper.getMaxItemCharge(inventory));
            int i = (int) (15 * charge);
    ...
    public boolean canConnectEnergy(ForgeDirection from) {
        return from == ForgeDirection.DOWN || (from != null && from.getOpposite()
            .ordinal() == facing);
    }
