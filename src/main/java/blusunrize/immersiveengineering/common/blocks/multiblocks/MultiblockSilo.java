...
public class MultiblockSilo implements IMultiblock {
    ...
    static {
        silo.formed = true;
        silo.pos = 4;
        for (int h = 0; h < 7; h++) for (int l = 0; l < 3; l++) for (int w = 0; w < 3; w++) {
            if (h == 0) {
                if ((l == 0 || l == 2) && (w == 0 || w == 2))
            ...
            } else if (h < 1 || h > 5 || w != 1 || l != 1) structure[h][l][w] = new ItemStack(
                IEContent.blockMetalDecoration,
                1,
                BlockMetalDecoration.META_sheetMetal);
        }
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        for (int yy = 0; yy <= 6; yy++) for (int xx = -1; xx <= 1; xx++) for (int zz = -1; zz <= 1; zz++) if (yy == 0) {
