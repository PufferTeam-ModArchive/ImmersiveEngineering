...
public class MultiblockBucketWheel implements IMultiblock {
    ...
    static {
        ...
        wheel.pos = 24;
        wheel.facing = 4;
        for (int h = 0; h < 7; h++) for (int l = 0; l < 7; l++) {
            ...
            else structure[h][l][0] = new ItemStack(
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        for (int h = -3; h <= 3; h++) for (int w = -3; w <= 3; w++) {
            ...
            if (curr instanceof TileEntityBucketWheel) {
                ...
                tile.formed = true;
                tile.pos = (w + 3) + (h + 3) * 7;
                tile.offset = new int[] { (side == 2 ? w : side == 3 ? -w : 0), h,
                    (side == 4 ? w : side == 5 ? -w : 0) };
            }
        }
        return true;
    ...
    public ItemStack[] getTotalMaterials() {
        return new ItemStack[] { new ItemStack(IEContent.blockStorage, 9, 7),
