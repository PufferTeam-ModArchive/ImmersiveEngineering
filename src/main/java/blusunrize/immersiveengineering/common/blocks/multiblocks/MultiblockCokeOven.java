...
public class MultiblockCokeOven implements IMultiblock {
    ...
    static {
        for (int h = 0; h < 3; h++) for (int l = 0; l < 3; l++)
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        for (int yy = -1; yy <= 1; yy++) for (int xx = xMin; xx <= xMax; xx++) for (int zz = zMin; zz <= zMax; zz++)
            if ((yy != 0 || xx != 0 || zz != 0) && (!world.getBlock(x + xx, y + yy, z + zz)
                .equals(IEContent.blockStoneDecoration) || world.getBlockMetadata(x + xx, y + yy, z + zz) != 1))
                return false;
