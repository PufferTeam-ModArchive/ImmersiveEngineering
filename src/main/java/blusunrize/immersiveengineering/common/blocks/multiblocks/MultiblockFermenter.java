...
public class MultiblockFermenter implements IMultiblock {
    ...
    static {
        for (int h = 0; h < 3; h++)
            for (int l = 0; l < 3; l++) for (int w = 0; w < 3; w++) structure[h][l][w] = (h == 1 && (w != 1 || l != 1))
        ...
        for (int l = 0; l < 3; l++) for (int w = -1; w <= 1; w++) for (int h = -1; h <= 1; h++) {
            ...
            if (h == 0 && (w != 0 || l != 1)) {
                if (!(world.getBlock(xx, yy, zz)
                    .equals(IEContent.blockMetalMultiblocks)
                    && world.getBlockMetadata(xx, yy, zz) == BlockMetalMultiblocks.META_fermenter)) return false;
            } else {
                if (!(world.getBlock(xx, yy, zz)
                    .equals(IEContent.blockMetalDecoration)
                    && world.getBlockMetadata(xx, yy, zz) == BlockMetalDecoration.META_lightEngineering)) return false;
            }
        }
