...
public class MultiblockBottlingMachine implements IMultiblock {
    ...
    static {
        bottle.pos = 4;
        bottle.formed = true;
        for (int h = 0; h < 2; h++) for (int l = 0; l < 2; l++) for (int w = 0; w < 3; w++) {
            if (l == 0 && w == 1) structure[h][l][w] = new ItemStack(
            ...
            else structure[h][l][w] = new ItemStack(IEContent.blockMetalDevice, 1, BlockMetalDevices.META_conveyorBelt);
        }
    }
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        if (b) for (int h = -1; h <= 0; h++) for (int l = 0; l < 2; l++) for (int w = -1; w <= 1; w++) {
            int ww = mirrored ? -w : w;
            ...
            world
                .setBlock(xx, yy, zz, IEContent.blockMetalMultiblocks, BlockMetalMultiblocks.META_bottlingMachine, 0x3);
    ...
    boolean structureCheck(World world, int startX, int startY, int startZ, int side, boolean mirror) {
        for (int h = -1; h <= 0; h++)
            for (int l = 0; l < 2; l++) for (int w = -1; w <= 1; w++) if (structure[h + 1][1 - l][w + 1] != null) {
                int ww = mirror ? -w : w;
                int xx = startX + (side == 4 ? l : side == 5 ? -l : side == 2 ? -ww : ww);
                int yy = startY + h;
                int zz = startZ + (side == 2 ? l : side == 3 ? -l : side == 5 ? -ww : ww);
                ...
                if (tile instanceof TileEntityConveyorBelt) {
                    ...
                    int fExpected = l == 1 ? (w == -1 ? ForgeDirection.OPPOSITES[side] : side)
