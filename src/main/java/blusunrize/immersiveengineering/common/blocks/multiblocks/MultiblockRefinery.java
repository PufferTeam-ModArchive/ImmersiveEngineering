...
public class MultiblockRefinery implements IMultiblock {
    ...
    static {
        ...
        ref.pos = 17;
        ref.facing = 4;
        for (int w = 0; w < 5; w++) for (int l = 0; l < 3; l++) for (int h = 0; h < 3; h++) {
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        for (int l = 0; l < 3; l++) for (int w = -2; w <= 2; w++) for (int h = -1; h <= 1; h++) {
            ...
            if (!(world.getBlock(xx, yy, zz)
                .equals(IEContent.blockMetalDecoration) && world.getBlockMetadata(xx, yy, zz) == m)) return false;
        }
        ...
        for (int l = 0; l < 3; l++) for (int w = -2; w <= 2; w++) for (int h = -1; h <= 1; h++) {
            ...
            if (curr instanceof TileEntityRefinery) {
                ...
                tile.offset = new int[] { (side == 4 ? l - 1 : side == 5 ? 1 - l : side == 2 ? -w : w), h + 1,
