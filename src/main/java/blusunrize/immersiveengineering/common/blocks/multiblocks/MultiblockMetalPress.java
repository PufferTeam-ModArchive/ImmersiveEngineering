...
public class MultiblockMetalPress implements IMultiblock {
    ...
    static {
        for (int h = 0; h < 3; h++) for (int l = 0; l < 3; l++) if (h == 0) structure[h][l][0] = new ItemStack(
            ...
            else structure[h][l][0] = new ItemStack(IEContent.blockMetalDevice, 1, BlockMetalDevices.META_conveyorBelt);
        } else if (h == 2 && l == 1) structure[h][l][0] = new ItemStack(
            IEContent.blockMetalDecoration,
            1,
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        if (world.getTileEntity(
            ...
            startY,
            startZ + (dir == 2 ? -1 : dir == 3 ? 1 : 0)) instanceof TileEntityConveyorBelt)
            dir = ForgeDirection.OPPOSITES[((TileEntityConveyorBelt) world.getTileEntity(
                startX + (dir == 4 ? -1 : dir == 5 ? 1 : 0),
                startY,
                startZ + (dir == 2 ? -1 : dir == 3 ? 1 : 0))).facing];

        for (int l = -1; l <= 1; l++) for (int h = -1; h <= 1; h++) {
            if (h == 1 && l != 0) continue;
            ...
            if (h == -1) {
                ...
                if (l == 0) {
                    if (!(world.getBlock(xx, yy, zz)
                        .equals(Blocks.piston) && world.getBlockMetadata(xx, yy, zz) == 0)) return false;
                } else {
                    if (!(world.getBlock(xx, yy, zz)
                ...
                if (!(world.getBlock(xx, yy, zz)
                    .equals(IEContent.blockMetalDecoration)
                    && world.getBlockMetadata(xx, yy, zz) == BlockMetalDecoration.META_heavyEngineering)) return false;
            }
        }
