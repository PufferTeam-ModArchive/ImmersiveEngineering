...
public class MultiblockCrusher implements IMultiblock {
    ...
    static {
        ...
        crush.pos = 17;
        crush.facing = 4;
        for (int h = 0; h < 3; h++) for (int l = 0; l < 5; l++) for (int w = 0; w < 3; w++) {
            if ((l == 0 && h == 2) || (l == 4 && h == 2) || (l == 4 && w == 2 && h > 0)) continue;
            if (l == 0) structure[h][l][w] = new ItemStack(
                IEContent.blockMetalDecoration,
                1,
                BlockMetalDecoration.META_lightEngineering);
            else if (l == 4) {
                if (h < 1) structure[h][l][w] = new ItemStack(
                    IEContent.blockMetalDecoration,
                    1,
                    BlockMetalDecoration.META_scaffolding);
                else if (h < 2 && w < 2) structure[h][l][w] = new ItemStack(
                    IEContent.blockMetalDecoration,
                    1,
                    BlockMetalDecoration.META_lightEngineering);
            } else if (h == 0) structure[h][l][w] = new ItemStack(
                IEContent.blockMetalDecoration,
                1,
                (l == 2 && (w == 0 || w == 1)) ? BlockMetalDecoration.META_lightEngineering
                    : BlockMetalDecoration.META_scaffolding);
            else if (h == 1) structure[h][l][w] = new ItemStack(
                IEContent.blockMetalDecoration,
                1,
                (w == 1 && l == 2) ? BlockMetalDecoration.META_lightEngineering : BlockMetalDecoration.META_fence);
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        if (world.getBlock(x, y - 1, z)
            ...
            && world.getBlock(x + (side == 4 ? 2 : side == 5 ? -2 : 0), y - 1, z + (side == 2 ? 2 : side == 3 ? -2 : 0))
                .equals(IEContent.blockMetalDecoration)
            && world.getBlockMetadata(
                x + (side == 4 ? 2 : side == 5 ? -2 : 0),
                y - 1,
                z + (side == 2 ? 2 : side == 3 ? -2 : 0)) == BlockMetalDecoration.META_lightEngineering) {
        ...
        if (b) {
            for (int l = 0; l < 3; l++) for (int w = -2; w <= 2; w++) for (int h = -1; h <= 1; h++) {
                ...
                if (curr instanceof TileEntityCrusher) {
                    ...
                    tile.offset = new int[] { (side == 4 ? l - 1 : side == 5 ? 1 - l : side == 2 ? -ww : ww), h + 1,
    ...
    boolean structureCheck(World world, int startX, int startY, int startZ, int side, boolean mirror) {
        for (int l = 0; l < 3; l++) for (int w = -2; w <= 2; w++) for (int h = -1; h <= 1; h++) {
            ...
            if (w == -2) {
                ...
                if (l < 2 && !(world.getBlock(xx, yy, zz)
                    .equals(IEContent.blockMetalDecoration)
                    && world.getBlockMetadata(xx, yy, zz) == BlockMetalDecoration.META_lightEngineering)) {
                    return false;
                }
                ...
                if (!(world.getBlock(xx, yy, zz)
                    ...
                    && world.getBlockMetadata(xx, yy, zz)
                        == ((w == 0 && (l == 0 || l == 1)) ? BlockMetalDecoration.META_lightEngineering
                            : BlockMetalDecoration.META_scaffolding))) {
                    return false;
                }
            } else if (h == 0) {
                if (!(world.getBlock(xx, yy, zz)
                    .equals(IEContent.blockMetalDecoration)
                    && world.getBlockMetadata(xx, yy, zz)
                        == ((l == 1 && w == 0) ? BlockMetalDecoration.META_lightEngineering
                            : BlockMetalDecoration.META_fence))) {
                    return false;
                }
            } else if (h == 1) {
                if (!world.getBlock(xx, yy, zz)
                    .equals(Blocks.hopper)) {
                    return false;
                }
            }
        }
