...
public class MultiblockSheetmetalTank implements IMultiblock {
    ...
    static {
        tank.formed = true;
        tank.pos = 4;
        for (int h = 0; h < 5; h++) for (int l = 0; l < 3; l++) for (int w = 0; w < 3; w++) {
            if (h == 0) {
                if ((l == 0 || l == 2) && (w == 0 || w == 2))
                    structure[h][l][w] = new ItemStack(IEContent.blockWoodenDecoration, 1, 1);
                else if (l == 1 && w == 1) structure[h][l][w] = new ItemStack(
                    IEContent.blockMetalDecoration,
                    1,
                    BlockMetalDecoration.META_sheetMetal);
            } else if (h < 1 || h > 3 || w != 1 || l != 1) structure[h][l][w] = new ItemStack(
                IEContent.blockMetalDecoration,
                1,
                BlockMetalDecoration.META_sheetMetal);
        }
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        for (int yy = 0; yy <= 4; yy++) for (int xx = -1; xx <= 1; xx++) for (int zz = -1; zz <= 1; zz++) if (yy == 0) {
            ...
            } else if (xx == 0 && zz == 0) if (!world.getBlock(startX + xx, startY + yy, startZ + zz)
    ...
    public ItemStack[] getTotalMaterials() {
        return new ItemStack[] { new ItemStack(IEContent.blockWoodenDecoration, 4, 1),
