...
public class MultiblockExcavator implements IMultiblock {
    ...
    static {
        ...
        exc.pos = 4;
        exc.facing = 3;
        for (int l = 0; l < 6; l++) for (int w = 0; w < 3; w++) for (int h = 0; h < 3; h++) {
            if (l < 5 && w == 1) continue;
            if ((l == 2 || l == 0) && w == 0 && h == 2) continue;
            int m = -1;
            if (l == 5) {
                if (w != 0 && (h != 1 || w != 1)) structure[h][l][w] = new ItemStack(IEContent.blockStorage, 1, 7);
                else m = h == 1 && w == 0 ? BlockMetalDecoration.META_lightEngineering
                    : BlockMetalDecoration.META_heavyEngineering;
            } else if (l == 4) {
                if (w == 0 && h == 1) structure[h][l][w] = new ItemStack(IEContent.blockStorage, 1, 7);
                else m = w == 2 ? BlockMetalDecoration.META_lightEngineering
                    : BlockMetalDecoration.META_heavyEngineering;
    ...
    public boolean createStructure(World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        // &&
        // world.getBlock(x+(side==4?2:side==5?-2:0),y-1,z+(side==2?2:side==3?-2:0)).equals(IEContent.blockMetalDecoration)
        // &&
        // world.getBlockMetadata(x+(side==4?2:side==5?-2:0),y-1,z+(side==2?2:side==3?-2:0))==BlockMetalDecoration.META_lightEngineering)
        // {
        ...
        if (b) {
            for (int l = 0; l < 6; l++) for (int w = -1; w <= 1; w++) for (int h = -1; h <= 1; h++) {
                if (l > 0 && w == 0) continue;
            ...
            if (MultiblockBucketWheel.instance
                .isBlockTrigger(world.getBlock(wheelX, startY, wheelZ), world.getBlockMetadata(wheelX, startY, wheelZ)))
                MultiblockBucketWheel.instance
                    .createStructure(world, wheelX, startY, wheelZ, ForgeDirection.ROTATION_MATRIX[0][side], player);
        }
        return b;
    }
    ...
    boolean structureCheck(World world, int startX, int startY, int startZ, int side, boolean mirror) {
        for (int l = 0; l < 6; l++) for (int w = -1; w <= 1; w++) for (int h = -1; h <= 1; h++) {
            ...
            if (OreDictionary
                .itemMatches(structure[h + 1][5 - l][w + 1], new ItemStack(IEContent.blockStorage, 1, 7), true)) {
                // Steelblocks should have OreDict checks
                if (!Utils.compareToOreName(checkStack, "blockSteel")) return false;
            } else if (!OreDictionary.itemMatches(structure[h + 1][5 - l][w + 1], checkStack, true)) return false;
        }
        return true;
    }
    ...
    public ItemStack[] getTotalMaterials() {
        return new ItemStack[] { new ItemStack(IEContent.blockStorage, 12, 7),
