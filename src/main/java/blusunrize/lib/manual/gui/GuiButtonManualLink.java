...
public class GuiButtonManualLink extends GuiButton {
    ...
    public GuiButtonManualLink(GuiManual gui, int id, int x, int y, int w, int h, String key, String localized,
        int pageLinked) {
        super(id, x, y, w, h, "");
        this.gui = gui;
        this.key = key;
    ...
    public void drawButton(Minecraft mc, int mx, int my) {
        ...
        this.field_146123_n = mx >= this.xPosition && my >= this.yPosition
        ...
        if (field_146123_n) {
            ...
            gui.drawHoveringText(
                Arrays.asList(gui.manual.formatEntryName(key) + ", " + (pageLinked + 1)),
                mx + 8,
                my + 4,
                font);
