...
public abstract class ManualPages implements IManualPage {
    ...
    public static class Image extends ManualPages {
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            int yOff = 0;
            for (int i = 0; i < resources.length; i++) if (resources[i] != null && !resources[i].isEmpty()) {
            ...
            if (localizedText != null && !localizedText.isEmpty()) ManualUtils
                .drawSplitString(manual.fontRenderer, localizedText, x, y + yOff, 120, manual.getTextColour());
    ...
    public static class Table extends ManualPages {
        ...
        public void initPage(GuiManual gui, int x, int y, List<GuiButton> pageButtons) {
            ...
            manual.fontRenderer.setUnicodeFlag(true);
            int l = localizedText != null ? manual.fontRenderer.listFormattedStringToWidth(localizedText, 120)
                .size() : 0;
            ...
            if (localizedTable != null) {
                ...
                if (bars != null) {
                    ...
                    for (int i = 0; i < bars.length; i++) {
                        ...
                        //
                        // gui.drawGradientRect(xx,y+textHeight-4,xx+1,y+textHeight+(manual.fontRenderer.FONT_HEIGHT+1)*localizedTable.length,
                        // col,col);
                        xx += 4;
                        textOff[i] = xx;
                    }
                ...
                for (int i = 0; i < localizedTable.length; i++) {
                    for (int j = 0; j < localizedTable[i].length; j++) if (localizedTable[i][j] != null) {
                        ...
                        // manual.getTextColour());
                        if (j != 0) {
                            int l = manual.fontRenderer.listFormattedStringToWidth(localizedTable[i][j], w)
                                .size();
                            ...
                            if (horizontalBars) {
                                ...
                                gui.drawGradientRect(
                                    ...
                                    x + 120,
                                    (int) ((y + textHeight + yOff + l * manual.fontRenderer.FONT_HEIGHT) / scale + 1),
                ...
                if (bars != null) for (int i = 0; i < bars.length; i++) gui.drawGradientRect(
                    textOff[i] - 4,
                    y + textHeight - 4,
                    textOff[i] - 3,
                    y + textHeight + yOff,
    ...
    public static class ItemDisplay extends ManualPages {
        ...
        public void initPage(GuiManual gui, int x, int y, List<GuiButton> pageButtons) {
            ...
            if (length > 0) {
                ...
                int lines = (length / lineSum * 2) + (length % lineSum / line0)
                    + (length % lineSum % line0 > 0 ? 1 : 0);
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            ...
            if (localizedText != null && !localizedText.isEmpty()) ManualUtils
                .drawSplitString(manual.fontRenderer, localizedText, x, y + yOffset, 120, manual.getTextColour());
        ...
        @Override
        public boolean listForSearch(String searchTag) {
            for (ItemStack stack : stacks) if (stack.getDisplayName()
                .toLowerCase()
    ...
    public static class Crafting extends ManualPages {
        ...
        public void recalculateCraftingRecipes() {
            this.recipes.clear();
            List cmRecipes = CraftingManager.getInstance()
                .getRecipeList();
            for (Object o : cmRecipes) if (o instanceof IRecipe) {
                for (int iStack = 0; iStack < stacks.length; iStack++) {
                    Object stack = stacks[iStack];
                    if (stack instanceof ItemStack[]) for (ItemStack subStack : (ItemStack[]) stack)
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            ...
            for (int i = 0; i < stacks.length; i++) {
                ...
                if (!rList.isEmpty() && recipePage[i] >= 0 && recipePage[i] < this.recipes.size()) {
                    int maxX = 0;
                    for (PositionedItemStack pstack : rList.get(recipePage[i])) if (pstack != null) {
            ...
            for (int i = 0; i < stacks.length; i++) {
                ...
                if (!rList.isEmpty() && recipePage[i] >= 0 && recipePage[i] < this.recipes.size()) {
                    for (PositionedItemStack pstack : rList.get(recipePage[i]))
                        if (pstack != null) if (pstack.getStack() != null) {
                            RenderItem.getInstance()
                                .renderItemAndEffectIntoGUI(
                                    manual.fontRenderer,
                                    ManualUtils.mc().renderEngine,
                            ...
                            RenderItem.getInstance()
                                .renderItemOverlayIntoGUI(
                                    ...
                                    x + pstack.x,
                                    y + totalYOff + pstack.y);
                            if (mx >= x + pstack.x && mx < x + pstack.x + 16
            ...
            manual.fontRenderer.setUnicodeFlag(uni);
            if (localizedText != null && !localizedText.isEmpty()) ManualUtils
                .drawSplitString(manual.fontRenderer, localizedText, x, y + totalYOff - 2, 120, manual.getTextColour());
        ...
        public void buttonPressed(GuiManual gui, GuiButton button) {
            ...
            if (r >= 0 && r < stacks.length) {
                ...
                if (recipePage[r] >= this.recipes.get(stacks[r])
                    .size()) recipePage[r] = 0;
                if (recipePage[r] < 0) recipePage[r] = this.recipes.get(stacks[r])
                    .size() - 1;
            }
        }
        ...
        public boolean listForSearch(String searchTag) {
            for (Object stack : stacks) {
                if (stack instanceof ItemStack[]) {
                    for (ItemStack subStack : (ItemStack[]) stack) if (subStack.getDisplayName()
                        .toLowerCase()
                        .contains(searchTag)) return true;
                } else if (stack instanceof ItemStack) {
                    if (((ItemStack) stack).getDisplayName()
                        .toLowerCase()
                        .contains(searchTag)) return true;
                } else if (stack instanceof String) {
                    if (ManualUtils.isExistingOreName((String) stack))
                        for (ItemStack subStack : OreDictionary.getOres((String) stack)) if (subStack.getDisplayName()
                            .toLowerCase()
                            .contains(searchTag)) return true;
                }
            }
    ...
    public static class CraftingMulti extends ManualPages {
        ...
        public void renderPage(GuiManual gui, int x, int y, int mx, int my) {
            ...
            if (!recipes.isEmpty() && recipePage >= 0 && recipePage < this.recipes.size()) {
                int maxX = 0;
                for (PositionedItemStack pstack : recipes.get(recipePage)) if (pstack != null) {
                    if (pstack.x > maxX) maxX = pstack.x;
                    gui.drawGradientRect(
                        x + pstack.x,
                        y + pstack.y,
            ...
            manual.fontRenderer.setUnicodeFlag(uni);
            if (localizedText != null && !localizedText.isEmpty()) ManualUtils
                .drawSplitString(manual.fontRenderer, localizedText, x, y + yOff + 2, 120, manual.getTextColour());
        ...
        @Override
        public boolean listForSearch(String searchTag) {
            for (PositionedItemStack[] recipe : this.recipes) for (PositionedItemStack stack : recipe) {
                if (stack.stack instanceof ItemStack[]) {
                    for (ItemStack subStack : (ItemStack[]) stack.stack) if (subStack.getDisplayName()
                        .toLowerCase()
                        .contains(searchTag)) return true;
                } else if (stack.stack instanceof ArrayList)
                    for (ItemStack subStack : (ArrayList<ItemStack>) stack.stack) {
                        if (subStack.getDisplayName()
                            .toLowerCase()
                            .contains(searchTag)) return true;
                    }
                else if (stack.stack instanceof ItemStack) {
                    if (((ItemStack) stack.stack).getDisplayName()
                        .toLowerCase()
                        .contains(searchTag)) return true;
    ...
    public static String addLinks(ManualInstance helper, GuiManual gui, String text, int x, int y, int width,
        List<GuiButton> pageButtons) {
    ...
    public static class PositionedItemStack {
        ...
        public ItemStack getStack() {
            if (displayList == null) {
                ...
                if (stack instanceof ItemStack) {
                    if (((ItemStack) stack).getItem()
                        .getHasSubtypes() && ((ItemStack) stack).getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                    ...
                    for (ItemStack subStack : (List<ItemStack>) this.stack) {
                        if (subStack.getItem()
                            .getHasSubtypes() && subStack.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                            ArrayList<ItemStack> list = new ArrayList<ItemStack>();
                            subStack.getItem()
                                .getSubItems(
                                    subStack.getItem(),
